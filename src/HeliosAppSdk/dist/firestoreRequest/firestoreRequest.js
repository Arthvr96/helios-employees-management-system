...
// const handleDeleteCycle = (filterCallback, lastCycle) => {
//   return __handleGetDocs__(dispositionsCycles).then((docs) => {
//     const cycles = [];
//     docs.forEach((el) => {
//       cycles.push(el.id);
//     });
...
//               }
//             }
//             __handleSetDoc__(dispositionsEmployees, user.id, newData)
//               .then(() => {
//                 if (lastCycle) {
//                   newData.sort((a, b) => {
//                     const date1 = new Date(a.slice(0, 10)).getTime();
...
//                   });
//                   __handleUpdateDoc__(stateApp.pathName, stateApp.segments.cycleState, {
//                     lastDate1: newData[0].slice(0, 10),
//                     lastDate2: newData[0].slice(11, 21),
//                   }).catch((error) => window.alert(error.code));
//                 }
//               })
//               .catch((error) => window.alert(error.code));
